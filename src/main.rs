fn main() {
    let input = "hcpjssql4kjhbcqzkvr2fivebpllzqbkhg
...
";
    ...
    for line in lines {
        let left = line.chars().find(|d| d.is_digit(10));
        let right = line.chars().rfind(|d| d.is_digit(10));

        if let Some(left_char) = left {
            if left_char.is_digit(10) {
                if let Some(l) = left_char.to_digit(10) {
                    sum += l * 10;
                }
            }
        }
        if let Some(right_char) = right {
            if right_char.is_digit(10) {
                if let Some(l) = right_char.to_digit(10) {
                    sum += l;
                }
            }
        }
